...
{
    /// <summary>
    /// fiMenuListener2 の概要の説明です。
    /// メニューの実行で起動するフィーチャー起動タイミング監視クラス
    /// </summary>
    public class TMenuListener2 : ToolStripMenuItem, IFeatureEventListener
    {
        #region 属性（シリアライズする）
        ...
        public void LinkFeature(FeatureBase target)
        {
            ...
            if (OwnerItem.GetType() == typeof(ToolStripMenuItem) || OwnerItem.GetType().IsSubclassOf(typeof(ToolStripMenuItem)))
            {
                ((ToolStripMenuItem)OwnerItem).DropDownOpening += new EventHandler(onOpenMenuItem);
            }
        }
