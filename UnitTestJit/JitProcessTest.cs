...
namespace UnitTestJit
{
    ...
    public class JitProcessTest
    {
        ...
        public void Test011_get_y1_and_z1_from_w1_assy_with_CoJoinFrom()
        {
            ...
            JitWork w1, y1, z1;
            st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, w1 = new JitWork
            {
                Subset = st.Model,                
                Name = $"w1",
                Next = (st.Model, A),
        ...
        public void Test010_CoJoinFrom()
        {
            ...
            dat = st.Model.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "w1", EventTypes.In, "10:00", "B"));
            Assert.AreEqual(st.Model.Engine().GetWorks(SINK).Count(), 0);

            st.Model.Engine().DoNext();
            dat = st.Model.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.AreEqual(dat.Count, 0);
            Assert.AreEqual(st.Model.Engine().GetWorks(SINK).Count(), 1);
        }
        ...
        public void Test009_CiKanbanReturn()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: i + 1), EventTypes.Out, new JitWork
                {
                    Subset = st.Model,
                    Name = $"w{(i + 1):0}",
                    Next = (st.Model, X),
        ...
        public void Test008_PULL_Kanban_reuse()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, new JitWork
                {
                    Subset = st.Model,
                    Name = $"w{(i + 1):0}",
                    Next = (st.Model, X),
        ...
        public void Test005_JointProcess_No3()
        {
            ...
            for (var i = 0; i < 1; i++)
            {
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 5), EventTypes.Out, new JitWork
                {
                    Subset = st.Model,
                    Name = $"y{(i + 1):0}",
                    Next = (st.Model, Y),
        ...
        public void Test003_JointProcess_No1()
        {
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "y2", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "y3", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "y1", EventTypes.In, "9:00"));
        ...
        public void Test002_Works_same_timing()
        {
            ...
            var today = TimeUtil.ClearTime(DateTime.Now);
            JitWork a, b, c;
            st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, a = new JitWork
            {
                Subset = st.Model,
                Name = "a",
                Next = (st.Model, X),
            ...
            st.Model.Engine().DoNext();
            var k = 0;
            var dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "a", EventTypes.In, "9:00"));
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:04
            Assert.IsTrue(CMP(dat[k++], "a", EventTypes.Out, "9:05"));
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05   // X:b903   Y:    Z:a905
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.In, "9:05"));
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:06"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05   // X:   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:06"));
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:06   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:07"));
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.Out, "9:09"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:07   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.Out, "9:09"));
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));
            ...
            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.In, "9:09"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:   Y:c909    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:13"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:13   // X:   Y:c909    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.In, "9:13"));

            st.Model.Engine().DoNext(); dat = st.Model.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:13   // X:   Y:    Z:a905+b909+c913
            Assert.IsTrue(dat.Count == 0);
        }
        ...
        public void Test001_tmStage_Basic()
        {
            ...
            //  1 a out->in
            st.Model.Engine().DoNext();
            var dat = st.Model.Engine().Events.Peeks(3).ToList();
            ...
            st.Model.Engine().DoNext();    //  4
            dat = st.Model.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(CMP(dat[0], "a", EventTypes.In, "9:01"));
            Assert.IsTrue(CMP(dat[1], "c", EventTypes.Out, "9:02"));
            Assert.IsTrue(CMP(dat[2], "b", EventTypes.Out, "9:03"));

            st.Model.Engine().DoNext();    //  5
            dat = st.Model.Engine().Events.Peeks(3).ToList();
            ...
            Assert.IsTrue(st.Model.Engine().GetWorks(st.Model.ChildProcesses["Y"]).Count() == 0);
            Assert.IsTrue(st.Model.Engine().GetWorks(st.Model.ChildProcesses["Z"]).Count() == 1);

            st.Model.Engine().DoNext();    // 14
            dat = st.Model.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(CMP(dat[0], "b", EventTypes.In, "9:05"));
            Assert.IsTrue(CMP(dat[1], "c", EventTypes.Out, "9:06"));

            st.Model.Engine().DoNext();    // 15
            dat = st.Model.Engine().Events.Peeks(3).ToList();
            ...
            st.Model.Engine().DoNext();    // 25
            dat = st.Model.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(dat.Count == 0, "c moved to Z them c have sunk because Z have not next process");
            Assert.IsTrue(st.Model.Engine().GetWorks(st.Model.ChildProcesses["X"]).Count() == 0);
            Assert.IsTrue(st.Model.Engine().GetWorks(st.Model.ChildProcesses["Y"]).Count() == 0);
            Assert.IsTrue(st.Model.Engine().GetWorks(st.Model.ChildProcesses["Z"]).Count() == 3);
        }
