...
namespace UnitTestJit
{
    ...
    public class JitProcessTest
    {
        ...
        public void Test012_CiSwitchNextLink()
        {
            ...
            st.Subset.ChildProcesses.Add(SINK = new JitProcess
            {
                Name = "SINK",
            });
            st.Subset.ChildProcesses.Add(X = new JitProcess
            {
                Name = "X",
            ...
            st.Subset.AddProcessLink(X, SINK);

            st.Subset.ChildProcesses.Add(Y = new JitProcess
            {
                Name = "Y",
            });
            st.Subset.AddProcessLink(Y, SINK);

            st.Subset.ChildProcesses.Add(B = new JitProcess // 分岐元
            {
                Name = "B",
            });
            st.Subset.AddProcessLink(B, X);
            st.Subset.AddProcessLink(B, Y);

            st.Subset.ChildProcesses.Add(A = new JitProcess  // 前工程
            {
                Name = "A",
            });
            st.Subset.AddProcessLink(A, B);
        }
        ...
        public void Test011_get_y1_and_z1_from_w1_assy_with_CoJoinFrom()
        {
            ...
            st.Subset.ChildProcesses.Add(B = new JitProcess  // 次工程（合流工程）
            {
                Name = "B",
            });
            st.Subset.ChildProcesses.Add(C = new JitProcess  // 分岐工程
            {
                Name = "C",
            });
            st.Subset.ChildProcesses.Add(Y = new JitProcess  // 横工程Y
            {
                Name = "Y",
            });
            st.Subset.ChildProcesses.Add(Z = new JitProcess  // 横工程Z
            {
                Name = "Z",
            });
            st.Subset.ChildProcesses.Add(SINK = new JitProcess // 消滅工程
            {
                Name = "SINK",
            });
            st.Subset.ChildProcesses.Add(D = new JitProcess // 分岐工程
            {
                Name = "D",
            });
            ...
            st.Subset.AddProcessLink(A, B); // A→B Push。但し、B.Co.JoinFromでJoinできるまで待つ
            st.Subset.AddProcessLink(B, C); // B→C Push。分岐工程への移動
            ...
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, y1 = new JitWork
            {
                Subset = st.Subset,
                Name = $"y1",
                Next = (st.Subset, Y),
            ...
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 2), EventTypes.Out, z1 = new JitWork
            {
                Subset = st.Subset,
                Name = $"z1",
                Next = (st.Subset, Z),
        ...
        public void Test010_CoJoinFrom()
        {
            ...
            st.Subset.ChildProcesses.Add(B = new JitProcess  // 次工程
            {
                Name = "B",
            });
            st.Subset.ChildProcesses.Add(Y = new JitProcess  // 横工程Y
            ...
            // 工程間リンク
            st.Subset.AddProcessLink(A, B); // A→B Push。但し、B.Co.JoinFromでJoinできるまで待つ
            st.Subset.AddProcessLink(B, SINK); // B→SINK Push
            ...
            dat = st.Subset.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "w1", EventTypes.In, "10:00", "B"));
            Assert.AreEqual(st.Subset.Engine().GetWorks(SINK).Count(), 0);

            st.Subset.Engine().DoNext();
            dat = st.Subset.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.AreEqual(dat.Count, 0);
            Assert.AreEqual(st.Subset.Engine().GetWorks(SINK).Count(), 1);
        }
        ...
        public void Test009_CiKanbanReturn()
        {
            ...
            JitKanban ka;
            var testid = 0;
            st.Subset.Engine().SendKanban(ka = new JitKanban
            {
                Subset = st.Subset,
            ...
            st.Subset.Engine().SendKanban(new JitKanban
            {
                Subset = st.Subset,
            ...
            for (var i = 0; i < 2; i++)
            {
                st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: i + 1), EventTypes.Out, new JitWork
                {
                    Subset = st.Subset,
                    Name = $"w{(i + 1):0}",
                    Next = (st.Subset, X),
        ...
        public void Test008_PULL_Kanban_reuse()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, new JitWork
                {
                    Subset = st.Subset,
                    Name = $"w{(i + 1):0}",
                    Next = (st.Subset, X),
        ...
        public void Test007_PULL_Kanban_too_late()
        {
            ...
            // 初期かんばんを投入
            var testid = 0;
            st.Subset.Engine().SendKanban(TimeUtil.Set(today, hour: 9, minute: 30), new JitKanban   // かんばん送るも、工程Xにはワークが無いので、なにもしない
            {
                Subset = st.Subset,
                PullFromProcessKey = "X",
            ...
            st.Subset.Engine().SendKanban(TimeUtil.Set(today, hour: 9, minute: 32), new JitKanban   // かんばん送るも、工程Xにはワークが無いので、なにもしない
            {
                Subset = st.Subset,
                PullFromProcessKey = "X",
                PullToProcessKey = "Y",
            ...
            {
                st.Subset.Engine().SendKanban(TimeUtil.Set(today, hour: 12, minute: 00), new JitKanban   // かんばん送るも、工程Xにはワークが無いので、なにもしない
                {
                    Subset = st.Subset,
        ...
        public void Test006_Case_Pull_Kanban_JIT_or_not()
        {
            ...
            // 初期かんばんを投入
            int testid = 0;
            st.Subset.Engine().SendKanban(TimeUtil.Set(today, hour: 8, minute: 0), new JitKanban   // かんばん送るも、工程Xにはワークが無いので、なにもしない
            {
                Subset = st.Subset,
                TestID = ++testid,
                PullFromProcessKey = "X",
            ...
            // Kanban2を Xに投入依頼
            st.Subset.Engine().SendKanban(TimeUtil.Set(today, hour: 9, minute: 4), new JitKanban
            {
                Subset = st.Subset,
                TestID = ++testid,
                PullFromProcessKey = "X",
        ...
        public void Test005_JointProcess_No3()
        {
            ...
            st.Subset.ChildProcesses.Add(JP = new JitProcessPriorityJoint
            {
                Name = "JP",
            });
            ...
            st.Subset.ChildProcesses.Add(X);
            JP.ChildProcesses.Add(X.ID);

            st.Subset.ChildProcesses.Add(Y);
            JP.ChildProcesses.Add(Y.ID);

            st.Subset.ChildProcesses.Add(JP);
            st.Subset.ChildProcesses.Add(Z = new JitProcess
            {
                Name = "Z",
            });
            st.Subset.ChildProcesses.Add(SINK = new JitProcess
            ...
            st.Subset.AddProcessLink(JP, Z);
            st.Subset.AddProcessLink(Z, SINK);
            ...
            for (var i = 0; i < 1; i++)
            {
                st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 5), EventTypes.Out, new JitWork
                {
                    Subset = st.Subset,
                    Name = $"y{(i + 1):0}",
                    Next = (st.Subset, Y),
        ...
        public void Test004_JointProcess_No2()
        {
            ...
            JP.ChildProcesses.Add(X);
            JP.ChildProcesses.Add(Y);    // Y is priority
        ...
        public void Test003_JointProcess_No1()
        {
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(99).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "y2", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "y3", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "y1", EventTypes.In, "9:00"));
            ...
            //--- inject Process instance named 'Y' at this timing ---------------------
            JP.ChildProcesses.Add(Y);   // Expected to replace dummy process to actual process
            //------------------------
        ...
        public void Test002_Works_same_timing()
        {
            var st = new JitStage();
            JitProcess X, Y, Z;
            st.Subset.ChildProcesses.Add(new[]
            {
                X = new JitProcess
                {
                    Name = "X",
            ...
            st.Subset.AddProcessLink(X, Y);
            st.Subset.AddProcessLink(Y, Z);

            var today = TimeUtil.ClearTime(DateTime.Now);
            JitWork a, b, c;
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, a = new JitWork
            {
                Subset = st.Subset,
                Name = "a",
                Next = (st.Subset, X),
            });
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, b = new JitWork
            {
                Subset = st.Subset,
                Name = "b",
                Next = (st.Subset, X),
            });
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 0), EventTypes.Out, c = new JitWork
            {
                Subset = st.Subset,
                Name = "c",
                Next = (st.Subset, X),
            ...
            st.Subset.Engine().DoNext();
            var k = 0;
            var dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:00"));
            Assert.IsTrue(CMP(dat[k++], "a", EventTypes.In, "9:00"));
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:04
            Assert.IsTrue(CMP(dat[k++], "a", EventTypes.Out, "9:05"));
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05   // X:b903   Y:    Z:a905
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.In, "9:05"));
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:06"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:05   // X:   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:06"));
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:06   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:07"));
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.Out, "9:09"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:07   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "b", EventTypes.Out, "9:09"));
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:b905    Z:a905
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));
            ...
            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:09"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:c906   Y:    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.In, "9:09"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:09   // X:   Y:c909    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.Out, "9:13"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:13   // X:   Y:c909    Z:a905+b909
            Assert.IsTrue(CMP(dat[k++], "c", EventTypes.In, "9:13"));

            st.Subset.Engine().DoNext(); dat = st.Subset.Engine().Events.Peeks(3).ToList(); k = 0;  // 9:13   // X:   Y:    Z:a905+b909+c913
            Assert.IsTrue(dat.Count == 0);
        }
        ...
        public void Test001_tmStage_Basic()
        {
            ...
            st.Subset.AddProcessLink("X", Y.ID);
            st.Subset.AddProcessLink(Y, Z);
            ...
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 1), EventTypes.Out, b = new JitWork
            {
                Subset = st.Subset,
                Name = "b",
                Next = (st.Subset, X),
            });
            st.Subset.Engine().Events.Enqueue(TimeUtil.Set(today, hour: 9, minute: 2), EventTypes.Out, c = new JitWork
            {
                Subset = st.Subset,
                Name = "c",
                Next = (st.Subset, X),
            ...
            //  1 a out->in
            st.Subset.Engine().DoNext();
            var dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            //  2 a in -> X
            st.Subset.Engine().DoNext();
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            Assert.IsTrue(CMP(dat[2], "c", EventTypes.Out, "9:02"));
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["X"]).Count() == 1);

            st.Subset.Engine().DoNext();    //  3
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            st.Subset.Engine().DoNext();    //  4
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(CMP(dat[0], "a", EventTypes.In, "9:01"));
            Assert.IsTrue(CMP(dat[1], "c", EventTypes.Out, "9:02"));
            Assert.IsTrue(CMP(dat[2], "b", EventTypes.Out, "9:03"));

            st.Subset.Engine().DoNext();    //  5
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["X"]).Count() == 0);
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["Y"]).Count() == 1);

            st.Subset.Engine().DoNext();    //  6
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["Y"]).Count() == 0);
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["Z"]).Count() == 1);

            st.Subset.Engine().DoNext();    // 14
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(CMP(dat[0], "b", EventTypes.In, "9:05"));
            Assert.IsTrue(CMP(dat[1], "c", EventTypes.Out, "9:06"));

            st.Subset.Engine().DoNext();    // 15
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            ...
            st.Subset.Engine().DoNext();    // 25
            dat = st.Subset.Engine().Events.Peeks(3).ToList();
            Assert.IsTrue(dat.Count == 0, "c moved to Z them c have sunk because Z have not next process");
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["X"]).Count() == 0);
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["Y"]).Count() == 0);
            Assert.IsTrue(st.Subset.Engine().GetWorks(st.Subset.ChildProcesses["Z"]).Count() == 3);
        }
