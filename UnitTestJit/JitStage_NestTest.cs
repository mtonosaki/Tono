    ...
    public class JitStage_NestTest
    {
        ...
        public void Test001()
        {
            ...
            foreach (var st in new[] { st1, st2 })
            {
                ...
                st.Model.AddProcessLink(B, C); // BÅ®C PushÅBï™äÚçHíˆÇ÷ÇÃà⁄ìÆ
                st.Model.AddProcessLink(C, SINK); // BÅ®SINK Push
                st.Model.AddProcessLink(D, SINK); // DÅ®SINK Push

                var today = TimeUtil.ClearTime(DateTime.Now);  // H:M:S:MSÇÇOÇ…Ç∑ÇÈ
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: tp[st], minute: 0), EventTypes.Out, tw[(st, "w1")] = new JitWork
                {
                    Subset = st.Model,
                    Name = $"w1",
                    Next = (st.Model, A),
                ...
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: tp[st], minute: 0), EventTypes.Out, tw[(st, "y1")] = new JitWork
                {
                    Subset = st.Model,
                    Name = $"y1",
                    Next = (st.Model, Y),
                ...
                st.Model.Engine().Events.Enqueue(TimeUtil.Set(today, hour: tp[st], minute: 2), EventTypes.Out, tw[(st, "z1")] = new JitWork
                {
                    Subset = st.Model,
                    Name = $"z1",
                    Next = (st.Model, Z),
