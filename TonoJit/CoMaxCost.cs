...
namespace Tono.Jit
{
    ...
    public class CoMaxCost : CoBase, IWorkInReserved
    {
        ...
        /// MAX制約値の変数名
        /// </summary>
        public JitVariable Name { get; set; } = JitVariable.From("Count");    // :Work.Cost[CostName] is cost
        ...
        {
            LinkedListNode<WorkEventQueue.Item> nextexit = Events.Find(ei.Work.NextProcess, EventTypes.Out, ":Work");
            if (nextexit != null)
            {
                TimeSpan ret = MathUtil.Min(TimeSpan.FromDays(999.9), nextexit.Value.Work.ExitTime - Now);
